...
# Define file paths
INPUT_PROMPT_FILE = 'default_user/variables.txt'
OUTPUT_JSON_FILE = 'default_user/formulas.json'
...
def read_input_prompt(file_path):
    """Read the input prompt from a text file."""
    try:
...
{
    "module1": {
        "line1": "'Revenue Planning Module'.'Revenue by Product Line' + 'Revenue Planning Module'.'Revenue by Business Unit'",
        "line2": "'Net Income' / 'Total Revenue'"
    },
    "module2": {
        "line3": "('Revenue by Product Line' - PREVIOUS('Revenue by Product Line')) / PREVIOUS('Revenue by Product Line')",
        "line4": "'Revenue by Product Line' + ('Revenue by Product Line' * 'Revenue Growth Rate')",
        "line5": "information needed",
        "line6": "'Operating Expense by Category'"
    },
    "module3": {
        "line7": "IF NOT ISBLANK('Headcount Data Input'.'Departure Date') AND 'Headcount Data Input'.'Departure Date' = PERIOD(CURRENTPERIODSTART()) THEN 1 ELSE 0"
    }
}
...
- Single quotes or double quotes inside the formulas remain intact.
